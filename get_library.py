...
def get_ant_tracker():
    return obj_tracker([cv2.pyrDown, adaptThreshold, getContours, partial(get_centroids_pyrdown)])

def get_leaf_tracker():
    return obj_tracker([cv2.pyrDown,  make_hsv, color_mask,getContours, partial(get_centroids_pyrdown, min_value=50, max_value = 500)])

def get_tracker(data):
    h,w = data.video_ds[0].shape[:2]
    lr = partial(LeftRightCallback, LeftRight(TrackableObject, 100, 100))
    return Tracker([get_ant_tracker(),get_leaf_tracker()], data, CentroidTrackerLR(maxDisappeared=10, lpos=w//8,rpos=w-(w//8)),cbs = [DrawRect(),WarpFrame([0, 0, w,h], four_point_transform), DrawVerticalLines(0, w)], cb_funcs =lr )
